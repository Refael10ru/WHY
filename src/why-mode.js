...
function resolveRealDir(flagDir) {
  try {
    ...
    return (resolved === home || resolved.startsWith(home + path.sep)) ? realDir : null;
  } catch (e) {
    return null;
  }
}
...
if (require.main === module) {
  ...
  process.stdin.on('end', () => {
    try {
      ...
      if (/^\/why-mode(?::[^\s]*)?\s+on\b/i.test(prompt)) safeCreateFlag(flagPath);
      else if (/^\/why-mode(?::[^\s]*)?\s+off\b/i.test(prompt)) safeUnlinkFlag(flagPath);
