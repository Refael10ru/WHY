...
test("plugin.json has hooks.SessionStart", () => {
	assert.ok(
		Array.isArray(plugin.hooks.SessionStart) &&
			plugin.hooks.SessionStart.length > 0,
	);
});

test("plugin.json has hooks.UserPromptSubmit", () => {
	assert.ok(
		Array.isArray(plugin.hooks.UserPromptSubmit) &&
			plugin.hooks.UserPromptSubmit.length > 0,
	);
});

test("plugin.json has hooks.PreToolUse", () => {
	assert.ok(
		Array.isArray(plugin.hooks.PreToolUse) &&
			plugin.hooks.PreToolUse.length > 0,
	);
});
...
function extractScriptPaths(obj) {
	...
	// WHY: split assignment out of while condition — biome noAssignInExpressions; matchAll
	// alternative needs ES2020 target which conflicts with our Node 18 baseline approach.
...
test("outputs [WHY] badge when flag present", (tmp) => {
	...
	assert.ok(
		...
		`expected [WHY] in stdout, got: ${JSON.stringify(result.stdout)}`,
	);
});
