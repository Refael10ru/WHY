...
test("Edit .json file passes without comment when flag active", (tmp) => {
	...
	assert.strictEqual(
		result.status,
		0,
		`expected pass but got exit ${result.status}: ${result.stderr}`,
	);
});
...
test("Write .json file passes without comment when flag active", (tmp) => {
	...
	const result = runHook(
		writePayload('{"version": "1.0.0"}', "tsconfig.json"),
		tmp,
	);
	assert.strictEqual(
		result.status,
...
test("Edit .lock file passes without comment when flag active", (tmp) => {
	fs.writeFileSync(path.join(tmp, ".why-mode"), "on");
	const result = runHook(
		editPayload("some lock content", "package-lock.json"),
		tmp,
	);
	assert.strictEqual(
		result.status,
...
test("Edit .js file still requires comment when flag active", (tmp) => {
	...
	assert.strictEqual(
		result.status,
		2,
		"expected block on .js file with no comment",
	);
});
